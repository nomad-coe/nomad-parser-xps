    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger=logger):
        ...
        for item in file_data:
            ...
            for dlabel in item['metadata']['data_labels']:
                ...
                channels_in_metainfo.append(dlabel['label'].lower().replace(' ', '_'))
            ...
            for i, label in enumerate(channels_in_metainfo):
                ...
                if hasattr(spectrum, label):
                    ...
                    logger.warn(
                        'spectrum channel is not supported by metainfo', data=dict(channel=label))
                    continue

                if n_values is None:
                    n_values = len(value)
                else:
                    if n_values != len(value):
                        logger.warn(
                            'number of spectra values is inconsistent', data=dict(channel=label))

            spectrum.n_values = n_values
